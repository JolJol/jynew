...
namespace ZZY_test.Middleware
{
    ...
    public static class ExcelToLua
    {
        ...
        //Name与subName的分隔符
        private static readonly char[] nameSplit = {'_'};
        ...
        public static void ExportSingleLuaFile(string path, string outDir)
        {
            ...
            string outPath = Path.Combine(outDir, sheet[0][1] + "Config.lua");
        ...
        public static string GenLuaFile(DataRowCollection sheet, int colNum, int rowNum)
        {
            ...
            sb.Append(
                "--[[\n本文本由编辑器自动生成，如需修改请先修改Excel表格后在使用Unity生成本文件\n\n金庸群侠传3D重置版\nhttps://github.com/jynew/jynew\n\n这是本开源项目文件头，所有代码均使用MIT协议。\n");
            ...
            foreach (var col in colDescList)
            {
                if (col.type == LuaFieldType.l_struct && col.subName.Length > 1)
                {
                    ...
                    sb.AppendFormat(
                        "\tfor _,t in pairs(v.{0}) do\n\t\tif type(t) == 'table' then\n\t\t\tsetmetatable(t,mt{0})\n\t\tend\n\tend\nend\n",
                        col.name);
                }
            }

            string str = "local configMgr = Jyx2:GetModule('ConfigMgr')\n" +
            ...
            switch (col.type)
            {
                ...
                default:
                    ...
                    for (int i = 0; i < vList.Length; i++)
                    {
                        ...
                        {
                            sb.Append(string.IsNullOrEmpty(v) ? "nul" : v);
                        }
